...
function crack($cphr, $text)
{
  ...
  $table ="abcdefghijklmnopqrstuvwxyz";
	// change key to lowercase for simplicity
  $cphr1 = strtolower($cphr);
  $cphr = str_replace(' ', '', $cphr);
  ...
  $text = str_replace(' ', '', $text);
	...
	for ($i = 0; $i < $length; $i++)
	{
        ...
        //variabel mengambil password ke-n
        $abcde = $cphr[$ki];
        $numCphr = $AbjToNumTable[$abcde];

        $angkaKey = ($numCphr-$numText)%26;
        $kataKey = substr($table,$angkaKey,1);
        $text[$i] = $kataKey;
			}
